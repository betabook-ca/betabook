...
The zone, DNS records, managed robots.txt, the `hello@betabook.ca` routing rule, the D1 database itself, and the R2 bucket are managed with OpenTofu in [`infra/cloudflare`](infra/cloudflare/README.md) and applied by Spacelift. The Worker, its bindings and secrets, and D1 migrations stay with wrangler.

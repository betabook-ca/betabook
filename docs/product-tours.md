...
The Journal tour covers Log, journal filters, Sends sorting, Goals (including tag filters and achievements), project history, Analytics, Find climbs, climber discovery, friend requests, Feed, and privacy. The sample Log control is a visual reference in the full tour, in a header row above the sample workspace as Log sits beside Search in the app, with no click action or popover. First-time invitations include an ordinary Log button. Update invitations and update demos omit both Log controls.
...
`lib/product-tour-demo.ts` defines Alex Morgan's browser-only fixtures. Journal entries are the source for sends, projects, and analytics; analytics use the production calculation. No database demo account is needed. Negative sample IDs must never enter entity links, real forms, or actions. The only tour mutation is saving the authenticated user's dismissal/completion status. `PrivacyFields` is shared with Account: its Send commentary selector offers Only me, Friends, Members, and Everyone, its Journal and goals selector offers the first three, and all fields use local state callbacks in the tutorial, while `PrivacyControls` owns real saving and error handling. The privacy lesson demonstrates member commentary with a Friends-only journal. A private profile disables both selectors and shows the effective Only me audience; switching back restores the saved choices. The commentary audience also protects the mirrored original-ascent note inside a visible journal. Its signed-in member summary lists the audiences separately. Keep the shared fields free of actions.

The Goals preview reuses `GoalForm`, `GoalSection`, `GoalDate`, `GoalCompletionNotice` and `FeedActivityCard` with fictional fixtures from `lib/product-tour-demo.ts`. Saving an example only updates local state. Sample log buttons demonstrate all-tags matching, and archiving retains the sample achievement in History and the friend's feed. The preview never imports goal mutation actions; sample profile links are disabled. Each goal lesson resets its example on navigation. The saved form and sample logs do not modify the signed-in account.
...
The journal tour is currently version 4. Version 1 introduced Journal, Sends, Projects, Analytics, and Account privacy. Version 2 added `find-climbers`, `friend-requests`, and `feed`, and substantially updated `account` for independent sharing audiences. Version 3 added `find-projects`, the Find climbs lesson. Version 4 adds `goals`, `goal-tags`, `goal-progress`, and `goal-achievements`, preserving all released lesson IDs. Version-3 users receive four Goals updates; version-2 users receive those plus Find climbs; version-1 users receive nine updates. First-time visits and Account replay include all fourteen lessons. `climber-search-preview.tsx` and `social-tour-previews.tsx` use local state with fictional people from `lib/product-tour-demo.ts`, without linking sample profiles or calling actions.

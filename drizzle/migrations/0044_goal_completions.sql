...
--> statement-breakpoint
CREATE TRIGGER goal_completions_journal_entries_update AFTER UPDATE OF user_id,climb_id,kind,is_ascent,entry_date ON journal_entries WHEN NEW.user_id IS NOT OLD.user_id OR NEW.climb_id IS NOT OLD.climb_id OR NEW.kind IS NOT OLD.kind OR NEW.is_ascent IS NOT OLD.is_ascent OR NEW.entry_date IS NOT OLD.entry_date
BEGIN
...
--> statement-breakpoint
CREATE TRIGGER goal_completions_goals_update AFTER UPDATE OF kind,target,discipline,grade,grade_match,timeframe,repeat,start_date,end_date,timezone ON goals WHEN NEW.kind IS NOT OLD.kind OR NEW.target IS NOT OLD.target OR NEW.discipline IS NOT OLD.discipline OR NEW.grade IS NOT OLD.grade OR NEW.grade_match IS NOT OLD.grade_match OR NEW.timeframe IS NOT OLD.timeframe OR NEW.repeat IS NOT OLD.repeat OR NEW.start_date IS NOT OLD.start_date OR NEW.end_date IS NOT OLD.end_date OR NEW.timezone IS NOT OLD.timezone
BEGIN
...
--> statement-breakpoint
CREATE TRIGGER goal_completions_goal_periods_update AFTER UPDATE OF goal_id,start_date,end_date,kind,target,discipline,grade,grade_match,repeat,timezone ON goal_periods WHEN NEW.goal_id IS NOT OLD.goal_id OR NEW.start_date IS NOT OLD.start_date OR NEW.end_date IS NOT OLD.end_date OR NEW.kind IS NOT OLD.kind OR NEW.target IS NOT OLD.target OR NEW.discipline IS NOT OLD.discipline OR NEW.grade IS NOT OLD.grade OR NEW.grade_match IS NOT OLD.grade_match OR NEW.repeat IS NOT OLD.repeat OR NEW.timezone IS NOT OLD.timezone
BEGIN

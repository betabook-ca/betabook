...
OpenTofu stack for the `betabook.ca` zone, DNS records, managed robots.txt, the `hello@betabook.ca` and `support@betabook.ca` Email Routing rules, the `betabook-db` D1 database, the `betabook-exports` R2 bucket that holds the weekly catalog snapshot, the `betabook-avatars` R2 bucket that holds uploaded profile photos, and the Turnstile widget that guards the auth forms. Spacelift plans pull requests and applies merges to `main`.

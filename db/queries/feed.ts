...
import { goalTitle, type GoalTitleInput } from "@/lib/goals";
...
export async function getFeedPage(
  ...
  const rows = await db.all<Row>(sql`
    WITH friends AS (
      ...
      UNION ALL
      SELECT g.user_id, completion.completed_date, completion.id, 'goal', NULL, NULL, NULL, completion.definition
    ...
    SELECT d.*, u.name, u.image, u.journalVisible AS journalVisible,
      p.id, p.kind, p.climbId, p.ascentStyle, p.goalDefinition,
  ...
  for (const row of rows) {
    ...
    day.activities.push({
      ...activity,
      goalTitle: goalDefinition ? goalTitle(JSON.parse(goalDefinition) as GoalTitleInput) : null,
      areaAncestors: [

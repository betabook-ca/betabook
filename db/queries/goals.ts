...
async function goalPeriodsQuery(
  ...
  const definitions = sql`
    WITH RECURSIVE initial_periods AS (
      ...
      SELECT g.*, min(${stop}, CASE g.repeat
      ...
      FROM initial_periods g WHERE g.ps<=${stop}
      UNION ALL
  ...
  const query = sql`${definitions}, contributions AS (
    ...
    FROM scoped_periods g JOIN goals live ON live.id=g.id
    LEFT JOIN ranked r ON r.id=g.id AND r.ps=g.ps AND r.repeat=g.repeat
...
type GoalRow = Omit<GoalProgress, "tags"> & { tags: string };
async function getGoalPeriods(
  ...
  if (!source) return [];
  return (await db.all<GoalRow>(source.query)).map((goal) => ({
    ...goal,
...
/** Share one live aggregate between both Goals views. */
...
export async function refreshGoalAchievements(db: Database, ownerId: string, now = new Date()) {
  ...
  // A concurrently introduced timezone waits for the next owner activity instead
  // of incorrectly baselining legacy history with an incomplete date lookup.
  const goalIds = sql`SELECT id FROM goals WHERE user_id=${ownerId}
    AND json_extract(${source.dates}, '$."' || timezone || '"') IS NOT NULL
    AND NOT EXISTS (SELECT 1 FROM goal_periods h WHERE h.goal_id=goals.id
      AND json_extract(${source.dates}, '$."' || h.timezone || '"') IS NULL)`;
  await db.batch([
    ...
    db
      ...
      .select(
        sql`SELECT NULL,current.id,current.periodStart,current.repeat,current.completedDate,json_object('kind',current.kind,'target',current.target,'discipline',current.discipline,'grade',current.grade,'repeat',current.repeat,'gradeMatch',current.gradeMatch,'tags',json(current.tags)) FROM (${completed}) current WHERE current.id IN (${goalIds})`,
      )
      .onConflictDoUpdate({
        target: [goalCompletions.goalId, goalCompletions.periodStart, goalCompletions.repeat],
        set: { completedDate: sql`excluded.completed_date`, definition: sql`excluded.definition` },
        setWhere: sql`goal_completions.completed_date IS NOT excluded.completed_date OR goal_completions.definition IS NOT excluded.definition`,
      }),

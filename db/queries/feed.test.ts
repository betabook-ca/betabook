...
it("formats stored period definitions with the shared goal title formatter", async () => {
  ...
  await refreshGoalAchievements(db, "public");
  const [completion] = await db.select().from(goalCompletions);
  expect(completion.definition).toMatchObject({
    kind: "training",
    target: 1,
    repeat: "none",
    tags: [],
  ...
  const page = await getFeedPage(db, "viewer");
  const activity = page.days.flatMap((day) => day.activities).find((item) => item.kind === "goal");
  expect(activity?.goalTitle).toBe(goalTitle(completion.definition));
});

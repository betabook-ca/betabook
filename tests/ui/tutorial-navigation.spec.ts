...
test("example navigation stays outside the scrolling lesson and switches cleanly at narrow widths", async ({
  ...
  if (info.project.name.startsWith("mobile")) {
    ...
    // Find climbs and Account settings, the same secondary rows as the app's menu.
...
for (const story of [
  ...
  test(`tutorial ${story} fits with visible primary navigation`, async ({ page }, info) => {
    ...
    // Desktop adds Find climbs and Account settings to the three primary destinations.
    await expect(nav.getByRole("link")).toHaveCount(info.project.name.startsWith("mobile") ? 3 : 5);
    const content = page.locator("[data-tour-scroll]");
